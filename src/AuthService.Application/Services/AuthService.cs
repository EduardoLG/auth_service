    ...
    public async Task<RegisterResponseDto> RegisterAsync(RegisterDto registerDto)
    {
        ...
        var user = new User
        {
            ...
            UserEmail = new UserEmail
            {
                ...
                EmailVerifiedTokenExpire = DateTime.UtcNow.AddHours(24)
            },
            UserRoles =
            [
...
// ENVIAR EMAIL (ESPERANDO EL RESULTADO)
try
{
    // Cambiamos el Task.Run por un AWAIT directo
    bool emailSent = await emailService.SendEmailAsync(user.Email, user.Username, emailVerificationToken);
    
    if (emailSent)
        logger.LogInformation("Verification email sent successfully to {Email}", user.Email);
    else
        logger.LogWarning("Email service returned false for {Email}", user.Email);
}
catch (Exception ex)
{
    // Ahora sí verás el error real en tu consola si Gmail rechaza la conexión
    ...
    public async Task<EmailResponseDto> VerifyEmailAsync(VerifyEmailDto verifyEmailDto)
    {
        ...
        user.UserEmail.EmailVerifiedTokenExpire = null;
        ...
        public async Task<EmailResponseDto> ResendVerificationEmailAsync(ResendVerificationDto resendDto)
    {
        ...
        user.UserEmail.EmailVerifiedTokenExpire = DateTime.UtcNow.AddHours(24);
        ...
        try
        {
            await emailService.SendEmailAsync(user.Email, user.Username, newToken);
            return new EmailResponseDto
            {
                Success = true,
        ...
        public async Task<EmailResponseDto> ForgotPasswordAsync(ForgotPasswordDto forgotPasswordDto)
    {
        ...
        else
        {
            ...
            user.PasswordReset.PasswordResetTokenExpiry = DateTime.UtcNow.AddHours(1); // 1 hora para resetear
        }
